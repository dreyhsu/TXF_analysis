...
def is_in_uptrend_period(bar_index, start_time, uptrend_periods_df):
    """
    Check if the given bar index falls within any uptrend period
    """
    current_time = start_time
    
    for _, period in uptrend_periods_df.iterrows():
...
def generate_signals(df, uptrend_periods_df=None):
    """
    Generate trading signals based on SuperTrend, only allowing entry signals during uptrend periods
    """
    ...
    for i in range(1, len(df)):
        current_signal = signals[i-1]  # Default to previous signal
        
        # Get current time for this bar
        if 'end_time' in df.columns:
            current_time = pd.to_datetime(df['end_time'].iloc[i])
        else:
            current_time = None
        
        # Check if we're in an uptrend period
        in_uptrend_period = True
        if uptrend_periods_df is not None and current_time is not None:
            in_uptrend_period = is_in_uptrend_period(i, current_time, uptrend_periods_df)
        
        # Generate signals based on SuperTrend
        if df['close'].iloc[i] > df['upperband'].iloc[i]:
            # Bullish signal - only allow entry if in uptrend period
            if in_uptrend_period or current_signal == 1:
                signals.append(1)
            else:
                signals.append(current_signal)
        elif df['close'].iloc[i] < df['lowerband'].iloc[i]:
            # Bearish signal - exit positions regardless of uptrend period
            signals.append(-1)
        else:
            signals.append(current_signal)
...
def main():
    ...
    try:
        ...
        for _, period in uptrend_periods_df.iterrows():
            print(f"  {period['start_time']} to {period['end_time']}")
    except FileNotFoundError:
        print(f"Warning: Uptrend periods file not found at {uptrend_periods_file}")
        print("Strategy will trade all signals without uptrend period filtering.")
        uptrend_periods_df = None
    ...
    try:
        ...
        raise FileNotFoundError(f"Volume bars file not found: {volume_bars_file}\n"
                                f"Please run 'python supertrend/create_volume_bars.py --asset TXF --volume {volume_per_bar}' first to generate volume bars.")
    ...
    # Generate the signals (with uptrend period filtering)
    supertrend_positions = generate_signals(supertrend_data, uptrend_periods_df)
    ...
    supertrend_df[['start_time', 'end_time', 'close', 'open', 'rsi_recovery', 'signals', 'trades']].to_csv(output_file, index=False)
